...
import com.zx.lab_attendance.utils.FastDFSClient;
import com.zx.lab_attendance.utils.FileUtils;
...
public class FileController {
    ...
    @PostMapping("/upload")
    @ResponseBody
    public JsonData upload (@RequestParam("base64Data") String base64Data, HttpServletResponse response, HttpServletRequest request) throws Exception {
        response.setHeader("Access-Control-Allow-Origin","*");
//        String dataPrix = ""; //base64格式前头
//        String data = "";//实体部分数据
        String uuid = UUID.randomUUID().toString().replaceAll("-", "");
        String imgPath = "/data/tmp" + uuid + "userface64.png";
        FileUtils.base64ToFile(imgPath,base64Data);

        //上传文件到fastdfs
        MultipartFile faceFile = FileUtils.fileToMultipart(imgPath);
        String url = fastDFSClient.uploadBase64(faceFile);
//        System.out.println(url);

        return JsonData.buildSuccess("http://39.101.193.187:88/zx/" + url);
